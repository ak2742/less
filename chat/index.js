...
const makeACall = () => {
    if (!el("videoPref").checked && !el("audioPref").checked) {
        alert("At least one of audio and video must be checked")
        return
    }
...
const startChat = (conn) => {
    ...
    el("sendMsgBtn").addEventListener('click', cb)
    document.addEventListener('keyup', e => e.code === "Enter" && cb())
